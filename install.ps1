$command1 = 'Function goinit {$p=Convert-Path .;$env:_EXISTINGPATH=$env:PATH; $env:_OLDGOPATH=$env:GOPATH; $env:GOPATH=$p; If ($env:_OLDGOPATH.length -gt 0) {$env:GOPATH+=";"+$env:_OLDGOPATH}; $env:PATH+=";"+$p+"\bin";}'
